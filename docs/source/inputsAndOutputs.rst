...
The pipeline also saves ``visualization/clustering/session_match_fraction.png``.
For each pair of sessions, it shows the fraction of one session's ROIs matched
to an ROI of the other, also plotted against the session gap. Wrong matches
count too, so it shows how often ROIs were linked, not whether the links are
right.
