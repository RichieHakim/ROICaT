...
from roicat import data_importing, helpers, ROInet, visualization
from roicat.tracking import alignment, scatteringWaveletTransformer, similarity_graph, cluster_assignment  ## sans blurring
...
folderName_inner = params['paths']['folderName_inner']

paths_allStat = np.array([path / folderName_inner / pathSuffixToStat for path in dir_allS2pFolders])[:]
paths_allOps  = np.array([path / folderName_inner / pathSuffixToOps for path in dir_allS2pFolders])[:]
