...
class Test_repr_methods:
    """Tests for __repr__ methods on key ROICaT classes."""

    def test_data_roicat_repr_empty(self):
        """Data_roicat repr works when no data has been loaded."""
        from roicat.data_importing import Data_roicat
        data = Data_roicat(verbose=False)
        r = repr(data)
        assert 'Data_roicat' in r
        assert 'n_sessions=0' in r
        assert 'n_roi_total=0' in r

    def test_data_roicat_repr_with_data(self):
        """Data_roicat repr shows correct counts after setting ROI images."""
        from roicat.data_importing import Data_roicat
        data = Data_roicat(verbose=False)
        ## Create minimal ROI images: 2 sessions, 3 and 5 ROIs
        roi_images = [
            np.random.rand(3, 10, 10).astype(np.float32),
            np.random.rand(5, 10, 10).astype(np.float32),
        ]
        data.set_ROI_images(roi_images, um_per_pixel=[1.0, 1.0])
        r = repr(data)
        assert 'Data_roicat' in r
        assert 'n_sessions' in r
        assert 'n_roi_total=8' in r

    def test_clusterer_repr(self):
        """Clusterer repr shows n_roi and nnz from similarity matrix."""
        ...
        s_sf = scipy.sparse.csr_matrix((vals, (rows, cols)), shape=(n, n))
        s_sf = (s_sf + s_sf.T) / 2
        ...
        c = clustering.Clusterer(
            s_sf=s_sf, s_NN_z=s_NN_z, s_SWT_z=s_SWT_z,
            s_sesh=s_sesh, verbose=False,
        )
        r = repr(c)
        assert 'Clusterer' in r
        assert 'n_roi=' in r
        assert 'nnz=' in r
        assert 'not fitted' in r

    def test_roi_graph_repr(self):
        """ROI_graph repr works immediately after init (before similarity computation)."""
        from roicat.tracking.similarity_graph import ROI_graph
        rg = ROI_graph(verbose=False)
        r = repr(rg)
        assert 'ROI_graph' in r
        assert 'n_roi=0' in r
        assert 'nnz=0' in r

    def test_roi_blurrer_repr(self):
        """ROI_Blurrer repr shows kernel width and blurred status."""
        from roicat.tracking.blurring import ROI_Blurrer
        rb = ROI_Blurrer(frame_shape=(64, 64), kernel_halfWidth=3, verbose=False)
        r = repr(rb)
        assert 'ROI_Blurrer' in r
        assert 'kernel_halfWidth=' in r
        assert 'blurred=False' in r
