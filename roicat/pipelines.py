...
def pipeline_tracking(params: dict, custom_data: data_importing.Data_roicat = None) -> tuple:
    ...
    ## Normalize aliases for data kinds. Historically, the tracking pipeline
    ## accepted the shorter ``'roicat'`` string for ROICaT richfile folders.
    ## Recent refactors renamed the internal option to ``'data_roicat'`` which
    ## broke existing configs and tests. Map these aliases (and the suite2p
    ## equivalent) back to the canonical names so both forms continue to work.
    data_kind_aliases = {
        'roicat': 'data_roicat',
